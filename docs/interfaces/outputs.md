...
Deck-switch additionally zeroes the _other pair's_ VU meters directly: `midi.sendShortMsg(0xBF, 0x44, 0)` and `midi.sendShortMsg(0xBF, 0x45, 0)`.
...
- **Rate** (`0x02`) passes `drop=2`, so nibbles 0-1 are dropped _inside_ the function, then the sign marker overwrites the new nibble 0 (original nibble 2) — 6 nibbles are sent, with a real sign marker.
